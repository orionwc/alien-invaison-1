...
controller.menu.onEvent(ControllerButtonEvent.Pressed, function () {
    ...
    if (AI) {
        ...
        game.setGameOverMessage(true, "YOU (DIDN'T) WIN!")
    } else {
...
game.showLongText("or.... (if you press Menu)", DialogLayout.Full)
...
game.onUpdateInterval(2000, function () {
    if (UFO) {
        if (AI) {
            controller.moveSprite(Unidentified_Flying_Object, 100, 100)
        } else {
            Unidentified_Flying_Object.setVelocity(randint(-100, 100), randint(-100, 100))
            controller.moveSprite(Unidentified_Flying_Object, 0, 0)
        }
    }
})
